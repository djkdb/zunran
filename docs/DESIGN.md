...
## 밸런스 진단 기록 (플레이 검증에서 잡은 것)

실제로 끝까지 플레이해 보고 헤드리스로 재현해 고친 구조적 결함들.

### 1. 슬롯 포화 → 성장 정지 (가장 컸다)

웨이브별 `총 DPS / 손님 체력배율` 을 재 보니:

```
wave  총DPS   평균티어  체력배율   여유도
  14     817    1.32      13.0     62.6
  15     886    1.29 ←    14.7     60.5   슬롯 21칸 포화
  20     986    1.29 ←    24.2     40.7   합성이 멈춘다
  25    1249    1.29      36.3     34.5
```

칸이 다 차면 뽑기가 막히고 → 같은 유닛 3개가 안 모이고 → **평균 티어가 1.29 에서
완전히 고정**된다. 그 뒤로 적 체력은 +147%, 내 화력은 +41%. 실제 플레이에서
체력이 20웨이브 동안 93 에 붙어 있다가 2웨이브 만에 죽었다.

이걸 아는 플레이어(칸을 비우는 `sellCommons` 전략)만 35웨이브를 갔고
모르는 플레이어는 27에서 죽었다 — 최적 플레이가 **숨은 요령**이었다.

→ 자동 정리(기본 켜짐)로 루프를 복구. 고친 뒤 평균 티어 1.56, 체력은
계단식으로 내려간다. 전략 격차도 8웨이브 → 2웨이브로 줄었다.

### 2. 메타 수당 인플레이션

`coinsEarned × 0.1` 이 수당의 95% 를 차지해 한 판에 5,000점이 들어왔다.
상점 전체가 8,800점이라 **두 판이면 다 샀다.** 게다가 코인은 후반에 남아돌아서
"더 멀리 갔다"를 보상하지 못했다.

→ `웨이브×14 + 처치×0.25 + 코인×0.004`. 웨이브 12면 249점, 29면 801점,
40이면 1,300점. 상점 전부 약 11판.

### 3. 런 제목 단조로움

콤보 문턱이 60인데 실제 콤보 중앙값이 92 라 **절반이 같은 제목**이었다.
측정된 중앙값 위로 문턱을 올리고(콤보 160, 합성 30, 전설 5), 웨이브 구간
제목을 촘촘하게 넣었다. 24판에 9종, 최빈값 29%.

### 측정 스크립트

```bash
npm run sim -- 14 autoClean            # 기본 플레이 전략
npm run sim -- 12 autoClean 0 x crowdNight   # 데일리 규칙별
