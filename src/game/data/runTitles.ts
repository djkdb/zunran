...
export const RUN_TITLES: RunTitleDef[] = [
  ...
  // ── 플레이 스타일 ──
  { id: 'combo', title: '콤보에 미친 야간 알바', priority: 62, when: (c) => c.stats.bestCombo >= 160 },
  { id: 'mergeAddict', title: '합성 중독', priority: 64, when: (c) => c.stats.merges >= 30 },
  { id: 'bossHunter', title: '보스 전문 알바', priority: 65, when: (c) => c.stats.bossKills >= 4 },
  { id: 'legendary', title: '전설만 다섯 번 왔다', priority: 66, when: (c) => c.stats.legendaryDraws >= 5 },
  { id: 'noMerge', title: '합성을 모르는 알바', priority: 62, when: (c) => c.wave >= 10 && c.stats.merges === 0 },
  { id: 'skillSpam', title: '셔터를 너무 자주 내렸다', priority: 58, when: (c) => c.stats.skillsUsed >= 34 },

  // ── 손님 쪽 사정 ──
  { id: 'karen', title: '진상 손님이 너무 많다', priority: 58, when: (c) => c.topEnemy === 'karen3am' },
  { id: 'zombie', title: '좀비가 편의점에 왔다', priority: 57, when: (c) => (c.stats.enemySeen.zombie ?? 0) >= 45 },
  { id: 'ramen', title: '라면 코너의 반란', priority: 56, when: (c) => (c.stats.enemySeen.ramen ?? 0) >= 40 },
  { id: 'bike', title: '오토바이는 못 잡는다', priority: 55, when: (c) => (c.stats.reachedBy.bikeCourier ?? 0) >= 3 },
  { id: 'runner', title: '뛰는 사람은 못 막는다', priority: 54, when: (c) => (c.stats.reachedBy.runner ?? 0) >= 9 },
  { id: 'student', title: '야자가 끝났다', priority: 52, when: (c) => (c.stats.enemySeen.student ?? 0) >= 55 },

  // ── 도달 웨이브 ──
  { id: 'earlyOut', title: '오늘은 조용하겠지', priority: 50, when: (c) => c.wave <= 5 },
  { id: 'shortShift', title: '오늘은 일찍 닫았다', priority: 48, when: (c) => c.wave >= 6 && c.wave <= 10 },
  { id: 'threeAm', title: '새벽 3시의 악몽', priority: 52, when: (c) => c.wave >= 13 && c.wave <= 17 },
  { id: 'pastDawn', title: '새벽을 넘겼다', priority: 50, when: (c) => c.wave >= 18 && c.wave <= 27 },
  { id: 'longNight', title: '긴 밤이었다', priority: 53, when: (c) => c.wave >= 28 && c.wave <= 37 },
  { id: 'dawn', title: '해 뜰 때까지 버텼다', priority: 70, when: (c) => c.wave >= 38 },
