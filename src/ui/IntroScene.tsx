...
// 말하는 사람 이름. 칸에 지정한 이름이 우선, 없으면 도감 이름.
function castName(defId: string, enemy: boolean, override?: string): string {
  if (override) return override;
  return (enemy ? ENEMY_BY_ID[defId]?.name : UNIT_BY_ID[defId]?.name) ?? "";
}
...
export function IntroScene({ onDone }: Props) {
  ...
  const panels: IntroPanel[] =
    name === null ? INTRO_PANELS : [...INTRO_PANELS, finalPanel(name)];
      ...
      <div className="intro-strip" ref={stripRef}>
        <div className="intro-strip-inner">
          {panels.slice(0, shown).map((p) => (
            <div
              key={p.id}
              className={`intro-panel k-${p.kind} ${p.shake && !reduced.current ? "shake" : ""}`}
            >
              {p.caption && <div className="intro-caption">{p.caption}</div>}
              <div className="intro-cast">
                {p.cast.map((c, i) => (
                  <div
                    className="intro-actor"
                    key={`${p.id}-${c.defId}-${i}`}
