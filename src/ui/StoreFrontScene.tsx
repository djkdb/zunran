...
const W = 360;
const H = 196; // 아래 56px 은 제목이 얹히는 자리라 비워 둔다
...
export function StoreFrontScene({ muted = false }: { muted?: boolean }) {
  ...
  useEffect(() => {
    ...
    const draw = (now: number) => {
      ...
      ctx.fillRect(0, 12, 74, 50);
      ctx.fillRect(126, 8, 58, 54);
      ctx.fillRect(248, 14, 88, 48);
      ...
      ctx.fillRect(0, 126, W, H - 126);
      ctx.fillStyle = '#1b1338';
      ctx.fillRect(0, 120, W, 7);

      // ── 편의점 본체 ──
      const sx = 62;
      const sw = 236;
      ctx.fillStyle = '#241c45';
      ctx.fillRect(sx, 44, sw, 76);
      // 유리창 (안쪽 조명)
      const glow = ctx.createLinearGradient(0, 56, 0, 118);
      glow.addColorStop(0, 'rgba(180,255,245,0.20)');
      glow.addColorStop(1, 'rgba(180,255,245,0.05)');
      ctx.fillStyle = glow;
      ctx.fillRect(sx + 8, 56, sw - 16, 58);
      // 창틀
      ctx.fillStyle = '#332a5c';
      for (let x = sx + 8; x < sx + sw - 8; x += 44) ctx.fillRect(x, 56, 2, 58);
      ctx.fillRect(sx + 8, 56, sw - 16, 2);
      // 안쪽 진열대 실루엣
      ctx.fillStyle = 'rgba(79,227,208,0.18)';
      ctx.fillRect(sx + 18, 88, 52, 12);
      ctx.fillRect(sx + 96, 88, 52, 12);
      ctx.fillRect(sx + 174, 88, 44, 12);
      // 카운터 안의 알바 (이 게임의 주인공)
      const alba = rasterize('alba') ?? rasterize('e_basic');
      if (alba) ctx.drawImage(alba, sx + 150, 74, 24, 24);
      // 출입문
      ctx.fillStyle = '#0d0a1c';
      ctx.fillRect(sx + sw / 2 - 18, 76, 36, 44);
      ctx.fillStyle = 'rgba(180,255,245,0.12)';
      ctx.fillRect(sx + sw / 2 - 15, 80, 30, 40);
      ...
      // 간판 뒤 발광
      const halo = ctx.createRadialGradient(sx + sw / 2, 34, 4, sx + sw / 2, 34, 96);
      halo.addColorStop(0, 'rgba(79,227,208,0.34)');
      halo.addColorStop(1, 'rgba(79,227,208,0)');
      ctx.fillStyle = halo;
      ctx.fillRect(sx - 60, 0, sw + 120, 76);
      ctx.fillStyle = '#0d0a1c';
      ctx.fillRect(sx + sw / 2 - 52, 22, 104, 24);
      ctx.strokeStyle = '#4fe3d0';
      ctx.lineWidth = 2;
      ctx.strokeRect(sx + sw / 2 - 52, 22, 104, 24);
      ctx.fillStyle = '#4fe3d0';
      ctx.font = 'bold 15px monospace';
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
      ctx.fillText('24H', sx + sw / 2, 35);
      ctx.restore();
      ...
      for (const w of walkers) {
        ...
        ctx.beginPath();
        ctx.ellipse(w.x, 127, w.scale * 0.3, w.scale * 0.09, 0, 0, Math.PI * 2);
        ctx.fill();
      }
